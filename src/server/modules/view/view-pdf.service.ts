...
export class ViewPdfService {
  ...
  private async renderPdf(reqUrl: string, sessionId: string): Promise<Buffer> {
    ...
    try {
      const result = await this.puppeteerService.renderPage(localUrl);
      this.logger.log(`Successfully created a pdf: Localized URL ${localUrl}, url: ${reqUrl} session id: ${sessionId}`);
      return result;
    } catch (e) {
      this.logger.error(`Failed to created a pdf: Localized URL ${localUrl}, url: ${reqUrl} session id: ${sessionId}`);
      throw e;
    }
  }
